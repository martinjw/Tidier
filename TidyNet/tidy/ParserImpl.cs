            ...
            public virtual void Parse(Lexer lexer, Node body, short mode)
            {
                ...
                while (true)
                {
                    ...
                    */
                    ...
                    //deprecated tags
                    if (((node.Tag.Model & ContentModel.Deprecated) != 0) && lexer.Options.WarnPresentational)
                    {
                        Report.Warning(lexer, node, node, Report.USING_PRESENTATION);
                        if (lexer.Options.MakeClean) continue;
                    }
            ...
            public virtual void Parse(Lexer lexer, Node element, short mode)
            {
                ...
                */
                ...
                while (true)
                {
                    ...
                    if (((node.Tag.Model & ContentModel.Deprecated) != 0) && lexer.Options.WarnPresentational)
                    {
                        Report.Warning(lexer, element, node, Report.USING_PRESENTATION);
                        if (lexer.Options.MakeClean) continue;
                    }
                ...
                */
                ...
                while (true)
                {
                    ...
                    if (node.Tag == tt.TagParam)
                    {
                        ...
                        //changed from warning to information. It's not significant that </param> is removed.
                        Report.Information(lexer, element, node, Report.DISCARDING_UNEXPECTED);
                        continue;
                    }
                    ...
                    */
                    ...
                    //closing embeds is demoted to info, to reduce youtube embed noise 
                    if (string.Equals(node.Element, "embed", StringComparison.OrdinalIgnoreCase))
                    {
                        Report.Information(lexer, element, node, Report.DISCARDING_UNEXPECTED);
                    }
                    else
