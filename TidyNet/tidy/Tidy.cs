...
namespace TidyNet
{
    ...
    public class Tidy
    {
        ...
        {
            //initialize options to my settings
            Options.WrapLen = 0; //no margin
            Options.Xhtml = true; //xml for tools
            Options.NoTableSummary = true; //by me, not supported in html5
            Options.TidyMark = false; //no meta generated by
            Options.DropEmptyParas = false; //empty p may be there for css
        ...
        /// <summary>
        /// Scans the specified directory for files ending in Options.Extension (normally .html)
        /// </summary>
        ...
        /// <summary>
        /// Loads the html from the specified path and return the tidied html.
        /// </summary>
        /// <param name="path">The path.</param>
        /// <param name="messages">The messages.</param>
        /// <returns></returns>
        public virtual string Load(string path, TidyMessageCollection messages)
        {
            var messageCollection = new TidyMessageCollection();
            var fi = new FileInfo(path);
            if (!fi.Exists || fi.Length > 500000) //sanity checking
            {
                return string.Empty;
            }
            //You can still load a binary file here. Don't.
            var html = File.ReadAllText(path);
            if (string.IsNullOrEmpty(html)) return null;
            var result = Parse(html, messageCollection);
            //merge in the messages and add the file path
            messageCollection.ForEach(x => x.Filename = path);
        ...
        public virtual string Parse(string input, TidyMessageCollection messages)
        {
            using (var stream = new MemoryStream())
            {
                using (var output = new MemoryStream())
                {
                    ...
                    stream.Position = 0;
                    Parse(stream, output, messages);
                    var html = Encoding.UTF8.GetString(output.ToArray());
                    //manually fix the double-lines issue
                    return html.Replace(">" + Environment.NewLine + Environment.NewLine + "<",
                        ">" + Environment.NewLine + "<");
                }
            }
        }
