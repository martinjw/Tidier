...
namespace TidyNet
{
    ...
    internal class AttVal : ICloneable
    {
        ...
        public virtual Attribute CheckAttribute(Lexer lexer, Node node)
        {
            ...
            if (attribute == null && _attribute.StartsWith("data-", StringComparison.OrdinalIgnoreCase))
            {
            ...
            if (attribute != null)
            {
                ...
                if (attribute.Presentational && lexer.Options.WarnPresentational)
                {
                    //images we allow to have width and height
                    var isImgWidthHeight = node.Tag == tt.TagImg && attribute.Name != "align";
                    if (!isImgWidthHeight)
                    {
                        Report.AttrError(lexer, node, Attribute, Report.USING_PRESENTATION);
                    }
                }
