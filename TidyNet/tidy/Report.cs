...
namespace TidyNet
{
    ...
    internal class Report
    {
        ...
        public const int UNKNOWN_INPUT_TYPE = 12;
        ...
        public const int USING_PRESENTATION = 32;
        ...
        public static void AttrError(Lexer lexer, Node node, string attr, short code)
        {
            ...
            else if (code == USING_PRESENTATION)
            {
                AddMessage(lexer, String.Format(GetMessage("presentational_attribute"), Tag(lexer, node), attr), MessageLevel.Warning);
            }
            else if (code == UNKNOWN_INPUT_TYPE)
            {
                var type = node.GetAttrByName("type");
                var typeValue = type != null ? type.Val : "";
                AddMessage(lexer, String.Format(GetMessage("unknown_input_type"), typeValue), MessageLevel.Warning);
            }
        ...
        public static void Information(Lexer lexer, Node element, Node node, short code)
        {
            //downgrade from warning to information
            if (code == DISCARDING_UNEXPECTED)
            {
                AddMessage(lexer, String.Format(GetMessage("discarding_unexpected"), Tag(lexer, node)), MessageLevel.Info);
            }
        }
        ...
        public static void Warning(Lexer lexer, Node element, Node node, short code)
        {
            ...
            {
                AddMessage(lexer, String.Format(GetMessage("badlayout_using_presentation"), Tag(lexer, node)), MessageLevel.Warning);
            }
        ...
        public static void Error(Lexer lexer, Node element, Node node, short code)
        {
            ...
            else if (code == UNEXPECTED_ENDTAG)
            {
                ...
                if (element != null)
                {
                    message = String.Format(GetMessage("unexpected_endtag_suffix"), element.Element, element.Element);
                }
                else
                {
                    message = String.Format(GetMessage("unexpected_endtag"), node.Element);
        ...
        public static void ErrorSummary(Lexer lexer)
        {
            ...
            if (lexer.badLayout != 0)
            {
                ...
                if ((lexer.badLayout & USING_PRESENTATION) != 0)
                {
                    AddMessage(lexer, GetMessage("badlayout_using_presentation"), MessageLevel.Warning);
                }
            }
        }
        ...
        public static void ReportVersion(Lexer lexer, Node doctype)
        {
            ...
            if (doctype != null)
            {
                ...
                if (lexer.Options.SuppressInfoMessages) return;
        ...
        {
            if (level == MessageLevel.Info && lexer.Options.SuppressInfoMessages) return;
